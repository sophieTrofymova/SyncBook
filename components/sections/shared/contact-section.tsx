...
export function ContactSection() {
  ...
  const handleSubmit = async () => {
    ...
    if (!name.trim() || !email.trim() || !description.trim() || emailIsInvalid) {
      ...
      <div className="mx-auto max-w-[1700px]">
        <div
          className="
            ...
            px-[25px] pb-5 pt-14
            md:rounded-[48px] md:px-16 md:py-16
          "
          style={{
            background:
              "linear-gradient(to bottom right, #2353f1, #4B74FF, #3D63E8)",
          }}
        >
            ...
            <div className="flex flex-col gap-3 mt-15 md:gap-8">
              ...
              <div>
                ...
                <input
                  ...
                  className={`${inputBase} ${emailIsInvalid
...
function MailBoxAnimation({ mailboxAlt }: { mailboxAlt: string }) {
  ...
  return (
    <div
      ref={wrapperRef}
      className="
  relative -ml-25 h-[280px] w-full max-w-[290px] overflow-visible
  md:h-[620px] md:w-[720px] md:max-w-none
"
    >
      <div className="absolute inset-0 z-30">
        {isVisible && (
          <img
            key={`${animationKey}-${isMobile ? "mobile" : "desktop"}`}
            src="/Letter.png"
            alt=""
            aria-hidden="true"
            className="absolute left-1/2 top-0 z-30 w-[195px] object-contain md:w-[250px]"
            style={{
              animationName: isMobile ? "mailFlyMobile" : "mailFly",
      ...
      <img
        ...
        alt={mailboxAlt}
        className="
          relative z-20 h-auto w-full object-contain
          drop-shadow-[0_24px_45px_rgba(0,0,0,0.35)]
          md:w-[720px]

          [mask-image:linear-gradient(to_bottom,transparent_0%,black_14%,black_82%,transparent_100%)]
          [-webkit-mask-image:linear-gradient(to_bottom,transparent_0%,black_14%,black_90%,transparent_100%)]
        "
      />
